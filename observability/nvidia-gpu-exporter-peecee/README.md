...
peecee is a *different host* (not in this repo's host scope), but the exporter exists only to
feed **proximal's** observability stack, so its canonical config lives here next to the scrape
job that consumes it (`prometheus/prometheus.yml`, job `gpu`, instance `peecee`).
