...
| [`exa/`](exa/) | hosted neural web search (Exa) | `exa-search` CLI wrapper over the Exa API, adopted 2026-09-08 as Council's primary search after beating wigolo 0.94 vs 0.54 nDCG@5; free tier, key in `~/.config/exa/env`; wigolo keeps fetch + fallback |
| [`wigolo/`](wigolo/) | local-first web-research layer (MCP) | keyless `wigolo` MCP server (Tavily replacement) with synthesis wired to native gemini-3.6-flash on the free tier (rewired 2026-08-06; GLM 5.2/OpenRouter before that, llama.cpp `:8081` before 2026-07-21); pre-adoption security audit, install/config posture, opt-in-feature guardrails |
...
| [`claude-oauth-refresh/`](claude-oauth-refresh/) | Claude Code OAuth token renewal | user timer (`*:0/15`) that renews `~/.claude` and `~/.claude-harm` access tokens 45 min before expiry, so headless consumers (agent-usage dashboard) never see the CLI's between-session token lapse; refresh-token rotation handled, file rewritten atomically |
